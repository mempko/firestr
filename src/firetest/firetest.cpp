...
po::options_description create_descriptions()
{
    ...
    const std::string port = "6061";
    const std::string fire_port = "6060";
    ...
    d.add_options()
        ...
        ("fire_port", po::value<std::string>()->default_value(fire_port), "port firestr is on");
...
int main(int argc, char *argv[])
{
    ...
    const std::string fire_port = vm["fire_port"].as<std::string>();

    fg::test_window w(host, port, fire_port);
    w.show();
