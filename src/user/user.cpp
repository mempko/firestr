...
namespace fire::user
{
    ...
    local_user_ptr load_user(const std::string& home_dir, const std::string& passphrase)
    {
        ...
        auto lu = std::make_shared<local_user>(
            info,
            contacts,
            greeters,
            introductions,
    ...
    bool contact_list::remove(user_info_ptr c)
    {
        ...
        auto ci = _map.find(c->id());
        if(ci == _map.end()) return false;

        //erase from list and map
        _list.erase(_list.begin() + ci->second);
        _map.erase(ci);
