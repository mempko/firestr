...
namespace fire::user
{
        ...
        _done{false}
    {
        ...
        for(const auto& c : _user->contacts().list())
            add_contact_data(c);
    ...
    void user_service::add_contact_data(const user::user_info_ptr u)
    {
    ...
    void user_service::request_register(const greet_server& g)
    {
        ...
        const auto tcp_s = n::make_tcp_address(g.host(), g.port());
        const auto udp_s = n::make_udp_address(g.host(), g.port());
    ...
    void user_service::received_ping(const message::message& m)
    {
        ...
        bool cur_state = false;
        {
            ...
            fire_event = 
                ...
                p->second.state == contact_data::CONNECTING;
    ...
    void user_service::received_connect_request(const message::message& m)
    {
        ...
        {
            LOG << "got connection request from: "
                << c->name()
                << " ("
                << c->address()
                << "), already connected..."
    ...
    void user_service::received_greet_find_response(const message::message& m)
    {
        ...
        auto external = 
            n::make_udp_address(rs.external().ip, rs.external().port);
    ...
    void user_service::update_contact_address(
            const std::string& id,
            const std::string& ip, n::port_type port)
    {
        INVARIANT(_user);
        INVARIANT(_encrypted_channels);
        u::mutex_scoped_lock l(_mutex);
    ...
    greet_servers::const_iterator find_greeter(
            const greet_servers& gs,
            const std::string& host,
            n::port_type port) 
    {
        return std::find_if(std::begin(gs), std::end(gs), 
                [&host, &port](const auto& g) 
                {
                    return g.host() == host && g.port() == port;
                });
    }
    ...
    bool greeter_exists(
            ...
            n::port_type port) 
    {
        return find_greeter(gs, host, port) != std::end(gs);
    }

    void user_service::add_greeter(
            const std::string& host,
            n::port_type port,
            const std::string& pub_key)
    {
        ...
        if(greeter_exists(_user->greeters(), host, port)) return;
    ...
    void user_service::remove_greeter(const std::string& address)
    {
        ...
        const auto host_port = n::parse_host_port(address);
        auto g = find_greeter(
                _user->greeters(),
                host_port.first,
                host_port.second);
    ...
    try
    {
        ...
        try
        {
            ...
            {
                const auto st = u::user_is_idle() ? IDLE : CONNECTED;
                s->send_ping(st);
                send_ticks = 0;
            } 
            else send_ticks++; 

            {
                ...
                for(auto& p : s->_contacts)
                {
                    ...
                    if(p.second.state == contact_data::OFFLINE || !p.second.contact) 
                        continue;
    ...
    void user_service::send_ping_requests()
    {
        ...
        {
            CHECK(c);
            if(is_contact_connecting(c->id()) || contact_available(c->id()))
                continue;
    ...
    void user_service::send_ping_request(
            const std::string& address,
            const sc::public_key& key,
            bool send_back)
    {
    ...
    void user_service::send_ping_request(us::user_info_ptr c, bool send_back)
    {
        ...
        for (const auto& address : c->addresses())
        {
            try
            {
                ...
                LOG << "sending connection request to "
                    << c->name()
                    << " ("
                    << c->id()
                    << ", "
            ...
            {
                LOG << "unknown error sending connection request."
                    << std::endl;
            }
        }
    }
    ...
    {
        const bool state_changed = contact_connected(id);
    ...
    bool has_id(const contact_introductions& is, const std::string& id)
    {
        return std::count_if(std::begin(is), std::end(is),
            [&id](const auto& i) 
            {
                return i.contact.id() == id;
            }) > 0; 
    }
